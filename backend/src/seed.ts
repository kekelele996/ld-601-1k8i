...
// 内存态种子数据，仓储直接变更这些数组以模拟持久化，刷新接口后结果保持一致。
export const seed = {
  userProfile: clone(seedData.userProfile),
  accessibleFacility: clone(seedData.accessibleFacility),
  routePlan: clone(seedData.routePlan),
  assistanceRequest: clone(seedData.assistanceRequest),
  barrierReport: clone(seedData.barrierReport)
};
