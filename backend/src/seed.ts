// 本地种子数据同时作为运行期内存数据源（本地评审禁止接入第三方 API / 外部数据库依赖）。
// 注意：不要加 as const，service 层在停用影响评估确认后需要原地变更这些记录。
...
// 进程级单例：后端运行期间的所有写操作都作用在这份内存数据上，重启后回到种子状态。
export const seed: SeedData = buildSeedData();
